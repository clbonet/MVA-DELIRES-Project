...
class autoencoder():
    ...
    def test(self,n_img=5):
        ...
        for t in range(T):
            ...
            output_noise = self.noiseModel.predict(curr_batch_blurred.reshape(n_img,self.img_rows,self.img_cols,1)).reshape(n_img,self.img_rows,self.img_cols)
            var += output_imgs**2 /T
...
if __name__ == '__main__':
    ...
    if(len(sys.argv)==1):
        ...
        ae.ae.save_weights('dense_combined_blurred.h5')
    else:
        if sys.argv[1] == "-l":
